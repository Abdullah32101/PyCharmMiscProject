...
class MySQLHelper:
    ...
    def insert_test_result(self, test_case_name, module_name, test_status, error_message=None, total_time_duration=None, device_name=None, screen_resolution=None, error_link=None):
    ...
    def store_test_result_in_tables(self, test_case_name, module_name, test_status, error_message=None, test_data=None, total_time_duration=None, device_name=None, screen_resolution=None, error_link=None):
        """Store test results in appropriate tables based on test type"""
        try:
            # Always store in test_results table
            self.insert_test_result(test_case_name, module_name, test_status, error_message, total_time_duration, device_name, screen_resolution, error_link)
            
            # Store in specific tables based on test type
            if 'book' in test_case_name.lower() or 'book' in module_name.lower():
                self._store_book_test_result(test_case_name, module_name, test_status, test_data)
            
            elif 'monthly' in test_case_name.lower() or 'monthly' in module_name.lower():
                self._store_subscription_test_result(test_case_name, module_name, test_status, 'monthly', test_data)
            
            elif 'six_month' in test_case_name.lower() or 'six_month' in module_name.lower():
                self._store_subscription_test_result(test_case_name, module_name, test_status, 'six_month', test_data)
            
            elif 'onetime' in test_case_name.lower() or 'onetime' in module_name.lower():
                self._store_onetime_test_result(test_case_name, module_name, test_status, test_data)
            
            elif 'user' in test_case_name.lower() or 'user' in module_name.lower():
                self._store_user_test_result(test_case_name, module_name, test_status, test_data)
            
            else:
                # Default: store as general order
                self._store_general_order_test_result(test_case_name, module_name, test_status, test_data)
                
        except Exception as e:
            print(f"❌ Error storing test result in tables: {e}")

    def _store_book_test_result(self, test_case_name, module_name, test_status, test_data=None):
        ...
        try:
            ...
            self.cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                order_number, user_id, book_id, order_type, amount,
                'credit_card', payment_status, order_status, datetime.now(),
    ...
    def _store_subscription_test_result(self, test_case_name, module_name, test_status, subscription_type, test_data=None):
        ...
        try:
            ...
            """, (
                order_number, user_id, order_type, amount,
                'credit_card', 'completed' if test_status == 'PASSED' else 'failed',
                'completed' if test_status == 'PASSED' else 'cancelled',
                datetime.now(), datetime.now() if test_status == 'PASSED' else None
            ))
    ...
    def _store_onetime_test_result(self, test_case_name, module_name, test_status, test_data=None):
    ...
    def _store_user_test_result(self, test_case_name, module_name, test_status, test_data=None):
        ...
        try:
            ...
            """, (
                ...
                'Test University', 'student', test_status == 'PASSED'
            ))
    ...
    def _store_general_order_test_result(self, test_case_name, module_name, test_status, test_data=None):
    ...
    def _get_or_create_test_user(self):
        ...
        try:
            ...
            self.cursor.execute("SELECT id FROM users WHERE username = 'test_user' LIMIT 1")
            ...
            self.cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                'test_user', 'test@example.com', 'test_hash', 'Test', 'User',
                'Test University', 'student', True
            ))
    ...
    def _get_or_create_test_book(self):
        ...
        try:
            ...
            self.cursor.execute("SELECT id FROM books WHERE title LIKE '%Test Book%' LIMIT 1")
            result = self.cursor.fetchone()
            ...
            self.cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, (
                'Test Book for Automation', 'Test Author', '978-TEST-1234',
                'Test Publisher', 2024, 49.99, 'Test book for automation testing',
                'Test Category', True
            ))
