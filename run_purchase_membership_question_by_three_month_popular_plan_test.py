...
def run_popular_test():
    """Run the popular plan test on multiple devices using pytest for database integration"""
    
    print("[📱] Starting popular plan test with mobile support and database integration...")
    ...
    if exit_code == 0:
        print("[✅] Popular plan test completed successfully with database logging!")
        return True
    else:
        print(f"[❌] Popular plan test failed with exit code: {exit_code}")
        return False

def run_mobile_popular_test():
    """Run the popular plan test specifically on mobile devices"""
    
    print("[📱] Starting mobile-specific popular plan test...")
    ...
    pytest_args = [
        ...
        "--capture=no",
        "-s",
        "--html=mobile_popular_test_report.html",
        "--self-contained-html"
    ]
    ...
    if exit_code == 0:
        print("[✅] Mobile popular plan test completed successfully!")
        return True
    else:
        print(f"[❌] Mobile popular plan test failed with exit code: {exit_code}")
        return False
