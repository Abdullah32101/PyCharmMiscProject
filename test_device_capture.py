...
def test_device_info_capture(driver, device_info):
    ...
    ), f"Device name should not be unknown, got: {device_info['name']}"
    assert (
        device_info["resolution"] != "unknown"
    ), f"Resolution should not be unknown, got: {device_info['resolution']}"

    print(
        f"[✅] Test passed - Device: {device_info['name']}, Resolution: {device_info['resolution']}"
    )
...
if __name__ == "__main__":
    ...
    # Run the test
    pytest_args = [__file__, "-v", "--tb=short", "--capture=no", "-s"]
