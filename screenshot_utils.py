...
class ScreenshotManager:
    ...
    def capture_screenshot(self, driver, test_name, stage="", error=False):
        ...
        if error:
            filename = (
                f"{test_name}_error_{stage}_{timestamp}.png"
                if stage
                else f"{test_name}_error_{timestamp}.png"
            )
        else:
            filename = (
                f"{test_name}_final_{stage}_{timestamp}.png"
                if stage
                else f"{test_name}_final_{timestamp}.png"
            )
    ...
    def capture_page_source(self, driver, test_name, stage="", error=False):
        ...
        if error:
            filename = (
                f"{test_name}_error_{stage}_{timestamp}.html"
                if stage
                else f"{test_name}_error_{timestamp}.html"
            )
        else:
            filename = (
                f"{test_name}_final_{stage}_{timestamp}.html"
                if stage
                else f"{test_name}_final_{timestamp}.html"
            )
