...
def capture_test_results(request, device_info):
    ...
    try:
        ...
        if hasattr(request, 'funcargs') and 'driver' in request.funcargs:
            ...
            if driver:
                ...
                if hasattr(request, 'funcargs') and 'driver' in request.funcargs:
                    driver_fixture = request._fixturemanager._arg2fixturedefs['driver'][0]
                    if hasattr(driver_fixture, 'cached_result') and driver_fixture.cached_result:
                        # Get the device name from the driver fixture's cached result
                        device_name = driver_fixture.cached_result[0].param
                        print(f"[📱] Got device name from driver fixture: {device_name}")
                
                # If we still don't have a device name, try to get it from the current request
                if device_name == "unknown" and hasattr(request, 'param') and request.param:
                ...
                if device_name == "unknown":
                    try:
                        ...
                        if "iPhone" in user_agent or viewport_width <= 414:
                            device_name = "iPhone"
                        elif "iPad" in user_agent or (viewport_width > 414 and viewport_width <= 1024):
                            device_name = "iPad"
    ...
    if hasattr(request.node, 'rep_call'):
        if request.node.rep_call.passed:
            ...
            status = "FAILED"
            error_message = str(request.node.rep_call.longrepr) if hasattr(request.node.rep_call, 'longrepr') else "Test failed"
        else:
            status = "ERROR"
    ...
    error_link = None
    if status in ["FAILED", "ERROR"] and hasattr(request, 'funcargs') and 'driver' in request.funcargs:
        driver = request.funcargs['driver']
        if driver:
            try:
...
def get_device_list():
    ...
    devices_env = os.getenv("TEST_DEVICES", "desktop,iPad Pro,iPhone X,Samsung Galaxy S21")
