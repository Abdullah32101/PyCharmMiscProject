...
def print_test_results(results):
    ...
    print(f"{'ID':<4} {'Test Case Name':<18} {'Module':<10} {'Status':<8} {'Duration':<8} {'Device':<8} {'Resolution':<12} {'DateTime':<16} {'Error Summary':<20}")
    print("-" * 130)
    ...
    for result in results:
        ...
        print(f"{result['id']:<4} {result['test_case_name'][:17]:<18} {result['module_name'][:9]:<10} {status_emoji} {result['test_status']:<6} {duration_str:<8} {device_emoji} {device_name[:6]:<6} {resolution_str:<12} {test_datetime:<16} {error_summary:<20}")
...
def show_module_breakdown(db_helper):
    ...
    try:
        ...
        print(f"{'Module Name':<25} {'Total':<8} {'Passed':<8} {'Failed':<8} {'Skipped':<8} {'Errors':<8}")
        print("-" * 75)
        ...
        for result in module_results:
            ...
            print(f"{result['module_name'][:24]:<25} {total:<8} {passed:<8} {failed:<8} {skipped:<8} {error:<8}")
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if command == "failed":
                ...
                print("❌ Unknown command. Available commands: failed, stats, modules, recent [limit]")
        else:
