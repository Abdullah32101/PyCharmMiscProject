...
class ComprehensiveResultsViewer:
    ...
    def show_books_summary(self):
        ...
        try:
            ...
            # Available books
            self.db_helper.cursor.execute(
                "SELECT COUNT(*) as count FROM books WHERE is_available = TRUE"
            )
            result = self.db_helper.cursor.fetchone()
    ...
    def show_recent_test_activity(self, limit=10):
        ...
        try:
            ...
            print(
                f"{'Test Case':<30} {'Module':<20} {'Status':<10} {'DateTime':<20} {'Order/Sub':<15}"
            )
            print("-" * 100)
            ...
            for result in results:
                ...
                order_sub = (
                    result["order_number"] or result["subscription_type"] or "N/A"
                )

                print(
                    f"{test_name:<30} {module_name:<20} {status_emoji} {result['test_status']:<8} {test_datetime:<20} {order_sub:<15}"
                )
    ...
    def show_test_type_breakdown(self):
        ...
        try:
            ...
            print(
                f"{'Test Category':<20} {'Total':<8} {'Passed':<8} {'Failed':<8} {'Success Rate':<12}"
            )
            print("-" * 60)
            ...
            for category in categories:
                ...
                print(
                    f"{category['test_category']:<20} {total:<8} {passed:<8} {failed:<8} {success_rate:.1f}%"
                )
        ...
        try:
            ...
            for i, test in enumerate(failed_tests, 1):
                ...
                if test["order_number"]:
                    print(
                        f"   Related Order: {test['order_number']} ({test['order_type']})"
                    )
