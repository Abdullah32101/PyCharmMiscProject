...
def run_all_membership_plan_tests():
    ...
    print(
        "[📱] Starting comprehensive membership plan tests with mobile support and database integration..."
    )
...
def run_specific_membership_plan_test(test_type):
    ...
    if exit_code == 0:
        ...
        print(
            f"[❌] {test_type} membership plan test failed with exit code: {exit_code}"
        )
        return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "mobile":
            ...
            print(
                "Available arguments: mobile, monthly, onetime, one-time, sixmonth, six-month, popular, three-month"
            )
            sys.exit(1)
    else:
        # Run all tests
