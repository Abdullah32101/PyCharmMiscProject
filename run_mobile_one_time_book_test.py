...
def run_mobile_book_test():
    ...
    print("[📱] Starting mobile one-time book purchase test with database integration...")
    ...
    if exit_code == 0:
        print("[✅] Mobile one-time book purchase test completed successfully with database logging!")
        return True
    else:
        print(f"[❌] Mobile one-time book purchase test failed with exit code: {exit_code}")
        return False
