...
from testing.screenshot_utils import screenshot_manager
...
def setup_database():
    ...
    try:
        ...
        print(f"WARNING: Database setup skipped due to connection issue: {e}")
        yield
...
def capture_test_results(request, device_info):
            ...
            if driver:
                ...
                if hasattr(request, "funcargs") and "driver" in request.funcargs:
                    ...
                    if (
                        ...
                        print(
                            f"[INFO] Got device name from driver fixture: {device_name}"
                        )
                ...
                if (
                    ...
                    print(f"[INFO] Got device name from request param: {device_name}")
                ...
                try:
                    ...
                    print(f"[INFO] Detected screen resolution: {screen_resolution}")
                except Exception as res_error:
                    print(f"[WARNING] Could not get screen resolution: {res_error}")
                ...
                if device_name == "unknown":
                    try:
                        ...
                        print(f"[INFO] Detected device type: {device_name}")
                    except Exception as device_error:
                        print(f"[WARNING] Could not detect device type: {device_error}")
                        device_name = "unknown"

    except Exception as e:
        print(f"[WARNING] Error getting device information: {e}")
    ...
    if (
        ...
        if driver:
            try:
                ...
                print(f"[WARNING] Failed to capture error screenshot: {screenshot_error}")
    ...
    try:
        print(
            f"[INFO] Storing test result: {cleaned_test_name} - {status} - Device: {device_name} - Resolution: {screen_resolution}"
        )
        helper = get_db_helper()
        ...
        print("[SUCCESS] Test result inserted successfully")
    except Exception as e:
        print(f"WARNING: Database storage skipped: {type(e).__name__} - {e}")
...
def get_device_list():
    ...
    if os.getenv("MOBILE_TEST", "false").lower() == "true":
        return ["iPhone X", "iPad Pro", "Samsung Galaxy S21"]  # Force mobile testing only

    # Get devices from environment variable or use default list
    devices_env = os.getenv(
        "TEST_DEVICES", "desktop,iPhone X,iPad Pro,Samsung Galaxy S21"
    )
    devices = [device.strip() for device in devices_env.split(",")]
    
    # Ensure all devices are included and desktop runs first
    default_devices = ["desktop", "iPhone X", "iPad Pro", "Samsung Galaxy S21"]
    
    # If TEST_DEVICES is set, use it; otherwise use default
    if devices_env == "desktop,iPhone X,iPad Pro,Samsung Galaxy S21":
        devices = default_devices
    else:
        # Ensure all default devices are included
        for device in default_devices:
            if device not in devices:
                devices.append(device)
    
    # Reorder to ensure desktop is first
    if "desktop" in devices:
        devices.remove("desktop")
        devices.insert(0, "desktop")
...
def driver(request):
    ...
    if HEADLESS:
        ...
        print("[INFO] Headless mode disabled")
    ...
    if device == "desktop":
        ...
        if device in device_mapping:
            ...
            print(f"[INFO] Running on mobile: {device}")
            ...
            chrome_options.add_experimental_option("mobileEmulation", mobile_emulation)
            print(f"[INFO] Running on Samsung Android: {device}")
        else:
            ...
            chrome_options.add_experimental_option("mobileEmulation", mobile_emulation)
            print(f"[INFO] Running on mobile with custom dimensions: {device}")
            _device_info["resolution"] = "375x812"  # Default mobile resolution

    # Add additional options for better stability and timeout handling
    ...
    # Improved timeout and stability settings
    chrome_options.add_argument("--no-sandbox")
    chrome_options.add_argument("--disable-dev-shm-usage")
    chrome_options.add_argument("--disable-gpu")
    ...
    if os.getenv('GITHUB_ACTIONS') == 'true':
        ...
        print(f"[SETUP] Using unique user data directory: {temp_dir}")

    # Create driver with improved timeout settings
    ...
    try:
        ...
        print(
            f"[INFO] Updated device info - Name: {_device_info['name']}, Resolution: {_device_info['resolution']}"
        )
    except Exception as e:
                    print(f"[WARNING] Could not get actual resolution: {e}")

    yield driver
    
    # Add delay before closing browser to see results
    import time
    print(f"[⏳] Test completed on {_device_info['name']}. Keeping browser open for 5 seconds...")
    time.sleep(5)
    
    # Clean up driver
    try:
        driver.quit()
        print(f"[✅] Browser closed successfully for {_device_info['name']}")
    except Exception as e:
        print(f"[⚠️] Error closing browser for {_device_info['name']}: {e}")
