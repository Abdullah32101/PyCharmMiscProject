...
from pages.purchase_membership_questions_by_six_month_plan_methods import (
...
def test_purchase_membership_questions_by_six_month_plan(driver, stage):
    ...
    try:
        ...
        print(
            f"[🚀] Starting six-month plan test for {stage} stage on {'Mobile' if is_mobile else 'Desktop'}"
        )
        ...
        with open(
            f"six_month_plan_error_{stage}_{int(time.time())}.html",
            "w",
            encoding="utf-8",
        ) as f:
